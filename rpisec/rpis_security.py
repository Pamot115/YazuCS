...
class RpisSecurity(object):
    """
    Lee y procesa configuración, verifica el sistema, la cámara y la interfaz de red
    """
    
    # Configuración a usar en caso de no encontrar el archivo .conf
    default_config = {
    ...
    def _read_data_file(self):
        """
        Lectura del archivo con chat-ids
        """
        result = None
        try:
    ...
    def save_telegram_chat_id(self, chat_id):
        """
        Si no hay chat-id, guarda el ID del chat que envie un mensaje
        """
        try:
            # Use a lock here?
    ...
    def _parse_config_file(self):
        """
        Conversión de string a parámetros de configuración
        """
        def _str2bool(v):
    ...
    def _check_system(self):
        """
        Check-up del sistema
        """
        # Ejecución como admin
        if not os.geteuid() == 0:
            exit_error('{0} must be run as root'.format(sys.argv[0]))
        
        # Lectura de interfaz de red en modo monitor para escaneo ARP
    ...
    def _check_monitor_mode(self):
        """
        Búsqueda de la subinterfaz monitor
        """
        result = False
        try:
    ...
    def _set_interface_mac_addr(self):
        """
        Obtención de la MAC de una interfaz
        """
        try:
    ...
    def _set_network_address(self):
        """
        Calcular la subred de la interfaz en modo monitor para buscar la MAC dentro de la misma subred
        """
    ...
    def telegram_send_message(self, message):
        # Envío de un mensaje por Telegram
    ...
    def telegram_send_file(self, file_path):
        # Envío de un archivo por Telegram
        ...
        try:
            ...
            if file_extension == '.mp4':
                for chat_id in self.saved_data['telegram_chat_ids']:
                    self.bot.sendVideo(chat_id=chat_id, video=open(file_path, 'rb'), timeout=200)
            elif file_extension == '.gif':
                for chat_id in self.saved_data['telegram_chat_ids']:
                    self.bot.sendDocument(chat_id=chat_id, document=open(file_path, 'rb'), timeout=200)
