...
def telegram_bot(rpis, camera):
    """
    Control del bot en base a la API de Telegram
    Se controla el envío y recepción de mensajes y archivos
    """
    ...
    def help(update, context):
        if check_chat_id(update):
            update.message.reply_text(parse_mode='Markdown', text='/help:\tAyuda sobre los comandos\n/estado:\tInformación sobre el tiempo de ejecución\n/detener:\tDetiene la instancia de ejecución actual\n/iniciar:\tHabilita el servicio de seguridad\n/foto:\tCaptura un fotograma al momento\n/gif:\tCaptura un gif de 2 segundos (toma un minuto en procesar)\n/reiniciar:\tReinicia el sistema', timeout=10)
    ...
    def disable(update, context):
        if check_chat_id(update):
            rpis.state.update_state('Deshabilitado')

    def enable(update, context):
        if check_chat_id(update):
            rpis.state.update_state('Detenido')
    ...
    try:
        ...
        dp.add_handler(CommandHandler("estado", status), group=3)
        dp.add_handler(CommandHandler("detener", disable), group=3)
        dp.add_handler(CommandHandler("iniciar", enable), group=3)
        dp.add_handler(CommandHandler("foto", photo), group=3)
        dp.add_handler(CommandHandler("gif", gif), group=3)
        dp.add_handler(CommandHandler("reiniciar", reboot), group=3)
