...
def parse_arguments():
    p = argparse.ArgumentParser(description='Un sistema de seguridad simple para Raspberry Pi.')
    p.add_argument('-c', '--config_file', help='Directorio del archivo de configuración.', default='/etc/yazu_cs.conf')
    p.add_argument('-s', '--data_file', help='Directorio del archivo de almacenamiento de chat-ids.', default='/var/lib/yazu_cs/data.yaml')
    p.add_argument('-d', '--debug', help='Habilitar modo de depuración', action='store_true', default=False)
    return p.parse_args()
...
def setup_logging(debug_mode, log_to_stdout):
    ...
    syslog_format = logging.Formatter("Yazu %(threadName)s %(message)s", "%Y-%m-%d %H:%M:%S")
...
if __name__ == "__main__":
    ...
    try:
        logger.info("Yazu running")
        rpis.telegram_send_message('Yazu se encuentra en ejecución')
        while True:
            time.sleep(100)
